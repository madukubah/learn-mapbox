		 ...
		 */
			public function do_multi_upload($field){
				...
				foreach ($_FILES[$field]["name"] as $i => $v) {
					...
					if(!empty( $this->_multi_file_name_override )){
						$this->file_name = $this->_prep_filename( $this->_multi_file_name_override )."_".$i;
						
						//If no extension was provided in the file_name config item, use the uploaded one.
						if(strpos($this->_multi_file_name_override , ".") === FALSE){
							$this->file_name .= $this->file_ext;
						//An extension was provided, lets have it!
						} else {
							$this->file_ext = $this->get_extension($this->_multi_file_name_override );
						}
				...
				if( $file_error_count>0 ){
					if( $file_error_count == $file_count ){
						return FALSE;
					}
					$this->set_error('upload_invalid_filetype_some_file');
				}
